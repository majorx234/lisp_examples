...
(defun tasks-find-by-huid ()
  (interactive)
  (let ((huid (if (use-region-p)
                  (buffer-substring-no-properties (region-beginning) (region-end))
                (completing-read "HUID: " nil)))
        (db-dir (tasks-find-database)))
    (if (not db-dir) ("task database was not found")
      (let ((task-md-path (file-name-concat db-dir huid "TASK.md")))
        (if (not (file-regular-p task-md-path)) ("HUID in database was not found")
          (find-file-other-window task-md-path))))))

(defun tasks-find-by-huid-select (beg end)
  (interactive (if (use-region-p)
                   (list (region-beginning) (region-end))
                 (list (point-min) (point-min)))
               (task-find-by-huid (buffer-substring-no-properties beg end))))
